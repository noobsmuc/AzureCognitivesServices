...
namespace noobsmuc.AzureCognitivesServices.AzureSpeech
{
    ...
    {
        private readonly string _AccessUri;
        ...
        public AzureAuthentication(string issueTokenUri, string apiKey)
        {
            _AccessUri = issueTokenUri;
            _ApiKey = apiKey;

            _AccessToken = HttpPost(issueTokenUri, _ApiKey);
        ...
        {
            string newAccessToken = HttpPost(_AccessUri, _ApiKey);
            //swap the new token with old one
            //Note: the swap is thread unsafe
            _AccessToken = newAccessToken;
            Console.WriteLine($"Renewed token for user: {_ApiKey} is: {_AccessToken}");
        }
